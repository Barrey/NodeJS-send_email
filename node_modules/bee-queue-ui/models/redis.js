...
var getActiveKeys = function(queueName){
    ...
    redis.keys("bq:" + queueName + ":active", function(err, keys) {
        dfd.resolve(keys);
    });
    return dfd.promise;
};
...
var getCompletedKeys = function(queueName){
    var dfd = q.defer();
    queueName = queueName ? queueName : '*';
    redis.keys("bq:" + queueName + ":succeeded", function (err, keys) {
        dfd.resolve(keys);
    });
    return dfd.promise;
};
...
var getFailedKeys = function(queueName){
    var dfd = q.defer();
    queueName = queueName ? queueName : '*';
    redis.keys("bq:" + queueName + "failed", function (err, keys) {
        dfd.resolve(keys);
    });
    return dfd.promise;
};
...
var getWaitingKeys = function(queueName){
    var dfd = q.defer();
    queueName = queueName ? queueName : '*';
    redis.keys("bq:" + queueName + ":waiting", function (err, keys) {
        dfd.resolve(keys);
    });
    return dfd.promise;
};
...
var getAllKeys = function(){
    var dfd = q.defer();
    redis.keys("bq:*:[0-9]*", function(err, keysWithLocks){
        var keys = [];
    ...
    if(list["keys"]){
        ...
        for(var i = 0, ii = objectKeys.length; i < ii; i++){
            for(var k = 0, kk = keys[objectKeys[i]].length; k < kk; k++){
                fullNames.push("bq:"+objectKeys[i]+":"+keys[objectKeys[i]][k]);
            }
        }
...
var getStatusCounts = function(){
    ...
    getStatus("active").done(function(active){
        getStatus("succeeded").done(function(completed){
            getStatus("failed").done(function(failed){
                getStatus("waiting").done(function(pendingKeys){
                    getAllKeys().done(function(allKeys){
                      redis.keys("bq:*:id", function (err, keys) {
                        var countObject = {
                            active: active.count,
...
var formatKeys = function(keys){
    ...
    getStatus("failed").done(function(failedJobs){
        getStatus("succeeded").done(function(completedJobs){
            getStatus("active").done(function(activeJobs){
                getStatus("waiting").done(function(pendingJobs){
                    var keyList = [];
                    for(var i = 0, ii = keys.length; i < ii; i++){
                        ...
                        else if(completedJobs.keys[explodedKeys[1]] && completedJobs.keys[explodedKeys[1]].indexOf(explodedKeys[2]) !== -1) status = "succeeded";
                        else if(failedJobs.keys[explodedKeys[1]] && failedJobs.keys[explodedKeys[1]].indexOf(explodedKeys[2]) !== -1) status = "failed";
                        else if(pendingJobs.keys[explodedKeys[1]] && pendingJobs.keys[explodedKeys[1]].indexOf(explodedKeys[2]) !== -1) status = "waiting";
...
var removeJobs = function(list){
    ...
    var multi = [];
    for(var i = 0, ii = list.length; i < ii; i++){
        var firstPartOfKey = "bq:"+list[i].type+":";
        multi.push(["del", firstPartOfKey+list[i].id]);
        multi.push(["lrem", firstPartOfKey+"active", 0, list[i].id]);
        multi.push(["lrem", firstPartOfKey+"waiting", 0, list[i].id]);
        multi.push(["srem", firstPartOfKey+"succeeded", list[i].id]);
...
var makePendingByType = function(type){
    ...
    var validTypes = ['active', 'succeeded', 'failed', 'waiting'];
    var dfd = q.defer();
    ...
    getStatus(type).done(function(allKeys){
        ...
        for(var i = 0, ii = allKeyObjects.length; i < ii; i++){
            var firstPartOfKey = "bq:"+allKeyObjects[i]+":";
            for(var k = 0, kk = allKeys.keys[allKeyObjects[i]].length; k < kk; k++){
                ...
                multi.push(["srem", firstPartOfKey+"succeeded", item]);
                multi.push(["srem", firstPartOfKey+"failed", item]);
                //Add to pending
                multi.push(["rpush", firstPartOfKey+"waiting", item]);
            }
        }
        redis.multi(multi).exec(function(err, data){
            if(err){
                ...
                dfd.resolve({success: true, message: "Successfully made all " + type + " jobs waiting."});
            }
        });
    });
    return dfd.promise;
...
var makePendingById = function(type, id){
    ...
    var firstPartOfKey = "bq:"+type+":";
    var multi = [];
    multi.push(["lrem", firstPartOfKey+"active", 0, id]);
    multi.push(["lrem", firstPartOfKey+"waiting", 0, id]);
    multi.push(["srem", firstPartOfKey+"succeeded", id]);
    multi.push(["srem", firstPartOfKey+"failed", id]);
    //Add to pending
    multi.push(["rpush", firstPartOfKey+"waiting", id]);
    redis.multi(multi).exec(function(err, data){
        if(err){
            dfd.resolve({success: false, message: err});
        }else{
            dfd.resolve({success: true, message: "Successfully made "+type+" job #"+id+" waiting."});
        }
    });
    return dfd.promise;
};
    ...
    getStatus(type, queueName).done(function(allKeys){
        ...
        for(var i = 0, ii = allKeyObjects.length; i < ii; i++){
            ...
            for(var k = 0, kk = allKeys.keys[allKeyObjects[i]].length; k < kk; k++){
                ...
                //Brute force remove from everything
                multi.push(["lrem", firstPartOfKey+"active", 0, item]);
                multi.push(["lrem", firstPartOfKey+"waiting", 0, item]);
                multi.push(["srem", firstPartOfKey+"succeeded", item]);
                multi.push(["srem", firstPartOfKey+"failed", item]);
...
var getDataById = function(type, id){
    ...
    var firstPartOfKey = "bq:"+type+":jobs";
    var multi = [];
    redis.hget(firstPartOfKey, id, function(err, data){
        if(err){
            dfd.resolve({success: false, message: err});
        }else{
            dfd.resolve({success: true, message: JSON.parse(data)});
        }
    });
    return dfd.promise;
};
...
var getProgressForKeys = function(keys){
    ...
    var multi = [];
    for(var i = 0, ii = keys.length; i < ii; i++){
        multi.push(["hget", "bq:"+keys[i].type+":"+keys[i].id, "progress"]);
    }
...
var getQueues = function(){
    var dfd = q.defer();
    var queues = redis.keysAsync("bq:*:id").then(function(queues) {
      return Promise.all(queues.map(function(queue) {
        ...
        var pending = redis.llenAsync(name + ":waiting");
        var delayed = redis.zcardAsync(name + ":delayed");
        var completed = redis.scardAsync(name + ":succeeded");
